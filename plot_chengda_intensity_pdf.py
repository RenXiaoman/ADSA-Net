...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "using Chengda nnUNet_train and nnUNet_val data."
        )
    )
    parser.add_argument(
        "--root",
        type=str,
        default="dataset/ChengdaOnlyCSPca",
        help="Root directory of the Chengda dataset.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/chengda_intensity_pdf",
...
def collect_case_paths(root: Path):
    image_dirs = [
        root / "nnUNet_train" / "imagesTr",
        root / "nnUNet_val" / "imagesTs",
    ]
    label_dirs = [
        root / "nnUNet_train" / "labelsTr",
        root / "nnUNet_val" / "labelsTs",
    ]
...
def collect_intensity_values(root: Path, fg_threshold: float):
    ...
    print(
        f"Found {len(image_paths)} image files across nnUNet_train and nnUNet_val, "
