...
from mosid_network_architecture.Attention import DisTransBlock, SE
from mosid_network_architecture.transunet.bottleneck_layer import Bottleneck
from mosid_network_architecture.transunet.decoder import Up, SignleConv
from mosid_network_architecture.vit.vit import ViT
...
class GraphLayer(nn.Module):
    def __init__(self, num_state, num_node, num_class):  # 64, 64, 64
        super().__init__()
...
class GCN(nn.Module):
    ...
    def forward(self, x):  # # x:  [2,1024,8,8]
        batch_size = x.size(0)
        # (N, C, THW)
        x = x.view(batch_size, x.size()[1], -1)  # [B, 1024, 64]
        x = x.permute(0, 2, 1)  # [B, 64, 1024]
        h = self.conv1(x)  # [B, 64, 1024]
        h = h + x
        h = self.relu(h)
        h = self.conv2(h)
        h = h.permute(0, 2, 1)
...
class GraphTransfer(nn.Module):
    ...
    def forward(self, word, vis_node):
        m_batchsize, C, Nc = word.shape  # [B, C, N]
        m_batchsize, C, Nn = vis_node.shape

        word = word.permute(0, 2, 1)  # [B, N, C]
        vis_node = vis_node.permute(0, 2, 1)  # [B, N, C]

        proj_query = self.query_conv(word)  # [B, N, C]
        proj_query = proj_query.view(m_batchsize, C, -1)  # [B, C, N]
        proj_query = proj_query.permute((0, 2, 1))  # [B, N, C]

        proj_key = self.key_conv(vis_node).view(m_batchsize, C, -1)  # [B, C, N]

        energy = torch.matmul(proj_query, proj_key)  # [B, N, N]
        attention_vis = self.softmax_vis(energy).permute((0, 2, 1))  # [B, N, N]
        attention_word = self.softmax_word(energy)  # [B, N, N]
...
class TransUnet_dis_graph_transfuse(nn.Module):
    ...
    def forward(self, x):
        ...
        fuse1 = self.trans_se1(torch.cat([conv_x1,trans_x1],dim=1))  # [B,256,64,64] → SE
        fuse1 = self.delchl1(fuse1)  #  通道降到128
        #fuse1 = self.mutual_trans1(conv_x1,fuse1)
        conv_x2 = self.layer1_Conv(fuse1 + conv_x1)  # [B,128,64,64]
        ...
        conv_x5 = self.layer4_Conv(fuse4)  # [B,1024,8,8]
        
        #############  瓶颈图推理  #############
        graph_fuse = self.GraphTrans(conv_x5, trans_vit3)  # 均为[2, 1024, 8, 8])
        #  内部做法（直觉）：
        #   - 把 conv_x5 / trans_vit3 视作图上的“可视节点(8×8=64个)”与“类节点（同为64）”
        #   - GCN 在节点维/状态维上做一维卷积信息传播
        #   - GraphTransfer 用 word(Q) × vis(K) 注意力把语义信息映射回空间节点
        #   - reshape 回 [B,1024,8,8] 并与原 conv_x5 残差相加
...
if __name__ == "__main__":
    ...
    deivice = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    
    a = torch.rand(2, 4, 128, 128).to(deivice)
    model = TransUnet_dis_graph_transfuse(in_channels=3, 
                                          img_dim=128, 
                                          vit_blocks=8, 
                                          vit_dim_linear_mhsa_block=512, 
                                          classes=4).to(deivice)
