...
class AttentionUnet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [B, C=1, D=16, H=256, W=256]
        print(f"输入形状: {x.shape}")
        x_m: torch.Tensor = self.head(x)  # [B, 64, 16, 256, 256]

        x_m = self.encdec(x_m)  # [5, 64, 16, 256, 256]

        x_m = self.reduce_channels(x_m)  # [5, 2, 16, 256, 256]
...
if __name__ == "__main__":
    ...
    spatial_dims = 3  # 3D数据
    in_channels = 1   # 输入通道数
    out_channels = 2  # 输出通道数：背景和病灶
    
    # 定义编码器通道数和下采样步长
    channels = [16, 32, 64, 128]  # 各层通道数
    strides = [2, 2, 2, 2]          # 各层下采样步长
    ...
    # 创建测试输入 [batch=5, channels=1, depth=16, height=256, width=256]
    test_input = torch.randn(5, 1, 16, 256, 256).to(device)
