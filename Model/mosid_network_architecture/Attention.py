...
class NonLocalBlockND(nn.Module):
    ...
    def forward(self, x,x1):
        ...
        g_x = g_x.permute(0, 2, 1)
        # print(g_x.shape)
        g_x1 = self.g(x1).view(batch_size, self.inter_channels, -1)  # [bs, c, w*h]
        ...
        theta_x = theta_x.permute(0, 2, 1)
        # print(theta_x.shape)
        theta_x1 = self.theta(x1).view(batch_size, self.inter_channels, -1)
        ...
        # print(f.shape)
        f1 = torch.matmul(theta_x1, phi_x)
...
class NonSingleModalBlock(nn.Module):
    """
    调用过程
    NONLocalBlock2D(in_channels=32),
    super(NONLocalBlock2D, self).__init__(in_channels,
    ...
    def __init__(self,
                 ...
                 sub_sample=True,
                 bn_layer=True):
        super(NonSingleModalBlock, self).__init__()
...
class DisTransBlock(nn.Module):
    def __init__(self,
                 ...
                 sub_sample=False,
                 bn_layer=True):
        super(DisTransBlock, self).__init__()
    ...
    def forward(self, x, x1):  # x 是conv, x1 是fuse
        '''
        :param x: (B, C,  H, W)
        :param x1: (B, C, H, W)
        :return:
        ...
        batch_size = x.size(0)  # 
        print(f'self.g(x) shape is : {self.g(x).shape}')
        
        #  [B, C, H, W] -> [B, C//2, H, W] -> [B, C//2, H * W]
        g_x = self.g(x).view(batch_size, self.inter_channels, -1)  # [bs, c, w*h]
        ...
        phi_x1 = self.phi(x1).view(batch_size, self.inter_channels, -1)  # [B, C//2, H * W]
...
class DisTransBlockv1(nn.Module):
    def __init__(self,
                 in_channels,
                 inter_channels=None,
                 dimension=2,
                 sub_sample=False,
                 bn_layer=True):
        super(DisTransBlockv1, self).__init__()
...
class DisTransBlockv3(nn.Module):
    def __init__(self,
                 in_channels,
                 inter_channels=None,
                 dimension=2,
                 sub_sample=False,
                 bn_layer=True):
        super(DisTransBlockv3, self).__init__()
        ...
        self.nolocal = NonSingleModalBlock(in_channels,dimension=dimension)
...
class Channel_Atten_1(nn.Module):
    """
    调用过程
    NONLocalBlock2D(in_channels=32),
    super(NONLocalBlock2D, self).__init__(in_channels,
    ...
    def __init__(self,
                 ...
                 sub_sample=False,
                 bn_layer=True):
        super(Channel_Atten_1, self).__init__()
    ...
    def forward(self, x,x1):
        ...
        f1 = torch.matmul(f_div_C, g_x)

        y = f1.permute(0, 2, 1).contiguous()
        y = y.view(batch_size, self.inter_channels, *x.size()[2:])
        W_y = self.W(y)
        return W_y
...
if __name__ == '__main__':
    ...
    #net = DisTransBlock(in_channels=64,dimension=2)
    net = Channel_Atten(in_channels=512,dimension=2)
    x2 = net(x)
    print(x2.shape)