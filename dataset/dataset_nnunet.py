...
class Lits_DataSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 预处理: DTCWT低频提取
        ADC_raw = process_nii_lowpass(ADC_raw)
        DWI_raw = process_nii_lowpass(DWI_raw)
        ...
        # # 获取3D小波变换的系数字典
        # coeffs_adc = pywt.dwtn(ADC, 'db2')
        # # 从字典中提取所需的系数
        # LL = coeffs_adc['aaa']  # 低频近似系数
        # LH = coeffs_adc['aad']  # 一个方向的高频系数
        # HL = coeffs_adc['ada']  # 另一个方向的高频系数
        # HH = coeffs_adc['add']  # 对角方向的高频系数
        # LH2 = coeffs_adc['daa'] # 另一个高频系数
        # HL2 = coeffs_adc['dad'] # 另一个高频系数
        # HH2 = coeffs_adc['dda'] # 另一个高频系数

        # adc_merge = LL + LH + HL + HH + LH2 + HL2 + HH2
        # ADC = self.z_score_normalization(adc_merge)

        # # 对DWI做相同处理
        # coeffs_dwi = pywt.dwtn(DWI, 'db2')
        # LL = coeffs_dwi['aaa']
        # LH = coeffs_dwi['aad']
        # HL = coeffs_dwi['ada']
        # HH = coeffs_dwi['add']
        ...
        # dwi_merge = LL + LH + HL + HH + LH2 + HL2 + HH2
        # DWI = self.z_score_normalization(dwi_merge)
