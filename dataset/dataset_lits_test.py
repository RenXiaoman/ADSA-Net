...
class Img_DataSet(Dataset):
    def __init__(self, adc, cut_param):
        ...
        # 直接使用完整图像，添加通道维度
        self.adc = adc[np.newaxis, :]  # [1, D, H, W]

    def __getitem__(self, index):
        return torch.from_numpy(self.adc).float()
...
def Test_all_Datasets(dataset_path, size):
    ...
    imagesTr = data_dir / 'prostate158_imagesTrVal'
    labelsTr = data_dir / 'prostate158_labelsTrVal'
    
    labels_list = sorted(list(labelsTr.glob("*.nii.gz")))
    labels_list = [f.name for f in labels_list]

    
    for file_name in labels_list:
        print("\nStart Evaluate: ", file_name)
        T2W = normalization(load(imagesTr / str(file_name).replace('.nii.gz', '_0000.nii.gz')) )
        original_shape = T2W.shape
        new_shape = cal_newshape(T2W, size)
        
        yield Img_DataSet(T2W, size), original_shape, new_shape, file_name
