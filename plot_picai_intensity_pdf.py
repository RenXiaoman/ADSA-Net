...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            ...
            "using PI-CAI images and lesion labels."
        )
    )
    parser.add_argument(
        "--root",
        type=str,
        default="dataset/PI-CAI",
        help="Root directory of the PI-CAI dataset.",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/picai_intensity_pdf",
...
def plot_modality_pdf(modality: str, tumor: np.ndarray, non_malignant: np.ndarray, output_dir: Path):
    ...
    save_path = output_dir / f"{modality}_pdf.png"
