...
DATASET_CONFIGS = {
    ...
    "prostate158": {
        "root": "dataset/Prostate158/nnUNet_train",
        "image_dirs": ["imagesTr", "imagesTs"],
        "label_dirs": ["labelsTr", "labelsTs"],
        "foreground_threshold": -1e9,
    },
    "chengda": {
        "root": "dataset/ChengdaOnlyCSPca",
        "image_dirs": ["nnUNet_train/imagesTr", "nnUNet_val/imagesTs"],
        "label_dirs": ["nnUNet_train/labelsTr", "nnUNet_val/labelsTs"],
        "foreground_threshold": -1e9,
    },
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Plot per-modality intensity distributions for csPCa vs non-malignant "
            "tissue using all datasets combined."
        )
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="output/all_datasets_intensity_pdf",
...
def collect_case_paths(root: Path, image_dirs: list[str], label_dirs: list[str]):
    label_map = {}
    for label_dir in label_dirs:
        full_label_dir = root / label_dir
        if not full_label_dir.exists():
            continue
        for label_path in full_label_dir.glob("*.nii.gz"):
    ...
    for image_dir in image_dirs:
        ...
        if not full_image_dir.exists():
            continue
        image_paths.extend(sorted(full_image_dir.glob("*.nii.gz")))
...
def collect_intensity_values_for_dataset(
    dataset_name: str,
    root: Path,
    image_dirs: list[str],
    label_dirs: list[str],
    ...
    image_paths, label_map = collect_case_paths(root, image_dirs, label_dirs)
    ...
    print(
        f"[{dataset_name}] Found {len(image_paths)} image files, "
        f"covering {len(matched_cases)} matched cases."
    )
    if missing_labels:
        print(
            f"[{dataset_name}] Warning: {len(set(missing_labels))} cases do not have labels "
            f"and were skipped."
        )
...
def merge_dataset_data(dataset_results: list[dict]):
    merged = {name: {"csPCa": [], "non_malignant": []} for name in MODALITY_MAP.values()}

    for result in dataset_results:
        for modality in MODALITY_MAP.values():
            for group_name in ["csPCa", "non_malignant"]:
                values = result[modality][group_name]
                if values.size > 0:
                    merged[modality][group_name].append(values)

    final = {}
    for modality, group_data in merged.items():
        final[modality] = {}
        for group_name, arrays in group_data.items():
            if arrays:
                final[modality][group_name] = np.concatenate(arrays, axis=0)
            else:
                final[modality][group_name] = np.array([], dtype=np.float32)
    return final
...
def plot_modality_pdf(modality: str, tumor: np.ndarray, non_malignant: np.ndarray, output_dir: Path):
    ...
    ax.set_title(f"{modality}: All Datasets Combined")
...
def main():
    ...
    for dataset_name, config in DATASET_CONFIGS.items():
        result = collect_intensity_values_for_dataset(
    ...
    all_data = merge_dataset_data(dataset_results)
